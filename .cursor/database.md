...
Use **Supabase Postgres**. This file mirrors the case study schema for local reference while implementing Domain / Infrastructure / migrations.

Source: [sinc-dev/developer-test-of-competence — database.md](https://github.com/sinc-dev/developer-test-of-competence/blob/main/database.md)
...
| Postgres table | `Crm.Domain` entity | Status |
|----------------|---------------------|--------|
| `profiles` | `Profile` | Done |
| `clients` | `Client` | Done |
| `conversation_threads` | `ConversationThread` | Done |
| `conversation_messages` | `ConversationMessage` | Done |
| `deals` | `Deal` | Done |
| `deal_stage_history` | `DealStageHistory` | Done |
| `deal_notes` | `DealNote` | Done |

| Postgres enum | `Crm.Domain` enum | Status |
|---------------|-------------------|--------|
| `app_role` | `AppRole` | Done |
| `conversation_status` | `ConversationStatus` | Done |
| `message_sender_type` | `MessageSenderType` | Done |
| `deal_stage` | `DealStage` | Done |
